...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        /*
         * String[] p1, p2; // Data origin if (args.length == 2) { // Use the given
         * arguments as data p1 = str2Array(args[0].trim().toUpperCase()); p2 =
         * str2Array(args[1].trim().toUpperCase()); } else { // Read the data from stdin
         * Scanner sc = new Scanner(System.in); System.out.print("p1? "); p1 =
         * str2Array(sc.nextLine().trim().toUpperCase()); System.out.print("p2? "); p2 =
         * str2Array(sc.nextLine().trim().toUpperCase()); sc.close(); } // Check if
         * error in the data if(result != null && result.equals("ERROR")) {
         * System.out.println("Result: " + result); return; } // Do the comparison and
    ...
    /**
     * Transform a data String into an array of Strings. Handle errors if not
     * containing 5 cards, or of a card is not a regular one (not a good value, not
     * a good suit). If encountering an error, we store "ERROR" in the result
     * string.
     * 
     * @param s the hand of the player, e.g., "1C 2C TD JH JS"
     * @return the hand as separated card encoded as strings, ["1C", "2C", "TD",
     *         "JH", "JS"]
     */
    ...
    /**
     * Find the highest combination in a hand. We detect flush (F), pair (P), and
     * highest card (H) for now. The result is the combination detected, followed by
     * its highest card (to avoid ex-aequo).
     * 
     * @param cs the hand, e.g., ["1C", "2C", "TD", "JH", "JS"]
     * @return the highest combination, e.g., "P J" (as the example contains a Pair
     *         of Jacks)
     */
    ...
    /**
     * Compare two combinations, and returns the winner among p1 or p2, or tie if
     * ex-aequo. We support full (F), pair (P), and highest card (H) for now.
     * 
    ...
    /**
     * When to hands contains the same combination, returns the highest one, or tie
     * if truly equivalent;
     * 
    ...
    /**
     * Build the comparaison table used to compare cards (ace is 1, two is 2, ...,
